...
type UserQuery struct {
	...
	withTrafficLogs *TunnelTrafficLogQuery
...
func (_q *UserQuery) QueryTrafficLogs() *TunnelTrafficLogQuery {
	query := (&TunnelTrafficLogClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) WithTrafficLogs(opts ...func(*TunnelTrafficLogQuery)) *UserQuery {
	query := (&TunnelTrafficLogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withTrafficLogs; query != nil {
		if err := _q.loadTrafficLogs(ctx, query, nodes,
			func(n *User) { n.Edges.TrafficLogs = []*TunnelTrafficLog{} },
			func(n *User, e *TunnelTrafficLog) { n.Edges.TrafficLogs = append(n.Edges.TrafficLogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadTrafficLogs(ctx context.Context, query *TunnelTrafficLogQuery, nodes []*User, init func(*User), assign func(*User, *TunnelTrafficLog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.TunnelTrafficLog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TrafficLogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_traffic_logs
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_traffic_logs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_traffic_logs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
